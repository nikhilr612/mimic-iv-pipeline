...
def read_diagnoses(mimic4_path):
    return read_diagnoses_icd_table(mimic4_path).merge(
        read_d_icd_diagnoses_table(mimic4_path),
        how="inner",
        left_on=["icd_code"],
        right_on=["icd_code"],
    )
...
def read_d_icd_procedures_table(mimic4_path):
    p_icd = dataframe_from_csv(
        os.path.join(mimic4_path, "hosp/d_icd_procedures.csv.gz")
    )
...
def read_procedures(mimic4_path):
    return read_procedures_icd_table(mimic4_path).merge(
        read_d_icd_procedures_table(mimic4_path),
        how="inner",
        left_on=["icd_code"],
        right_on=["icd_code"],
    )
...
def preproc_meds(module_path: str, adm_cohort_path: str) -> pd.DataFrame:
    ...
    med = med.dropna()
    dropped_count = original_count - len(med)
    if dropped_count > 0:
        logger.warning(f"Dropped {dropped_count:,} medication records due to missing values")

    logger.info(f"Medication preprocessing summary:")
    logger.info(f"  - Unique drug types: {med.itemid.nunique():,}")
    logger.info(f"  - Unique admissions: {med.stay_id.nunique():,}")
    logger.info(f"  - Total records: {med.shape[0]:,}")
...
def preproc_proc(
    ...
    original_count = len(df_cohort)
    df_cohort = df_cohort.dropna()
    dropped_count = original_count - len(df_cohort)
    if dropped_count > 0:
        logger.warning(f"Dropped {dropped_count:,} procedure records due to missing values")

    logger.info(f"Procedure preprocessing summary:")
    logger.info(f"  - Unique events: {df_cohort.itemid.dropna().nunique():,}")
    logger.info(f"  - Unique admissions: {df_cohort.stay_id.nunique():,}")
    logger.info(f"  - Total records: {df_cohort.shape[0]:,}")
...
def preproc_out(
    ...
    dropped_count = original_count - len(df_cohort)
    if dropped_count > 0:
        logger.warning(f"Dropped {dropped_count:,} output event records due to missing values")

    logger.info(f"Output events preprocessing summary:")
    logger.info(f"  - Unique events: {df_cohort.itemid.nunique():,}")
    logger.info(f"  - Unique admissions: {df_cohort.stay_id.nunique():,}")
    logger.info(f"  - Total records: {df_cohort.shape[0]:,}")
...
def preproc_chart(
    ...
    for chunk in tqdm(
        ...
        chunk_merged["event_time_from_admit"] = (
            chunk_merged[time_col] - chunk_merged["intime"]
        )
    ...
    logger.info(f"Chart events preprocessing summary:")
    logger.info(f"  - Unique events: {df_cohort.itemid.nunique():,}")
    logger.info(f"  - Unique admissions: {df_cohort.stay_id.nunique():,}")
    logger.info(f"  - Total records: {df_cohort.shape[0]:,}")
...
def preproc_icd_module(
    ...
    if icd_map_path:
        ...
        logger.info(f"ICD code mapping summary:")
        logger.info(f"  - Unique ICD-9 codes: {module[module['icd_version'] == 9]['icd_code'].nunique():,}")
        logger.info(f"  - Unique ICD-10 codes: {module[module['icd_version'] == 10]['icd_code'].nunique():,}")
        logger.info(f"  - Unique ICD-10 codes (after converting ICD-9): {module['root_icd10_convert'].nunique():,}")
        print(
